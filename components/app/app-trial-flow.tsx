...
function daysBetween(start: string, end: string) { return Math.max(1, Math.round((dateValue(end) - dateValue(start)) / 86_400_000)); }
...
export function AppTrialFlow({ proofWindow, baseline, followupReady, receiptId, campaign, enrollment, reward, scenario, demoMode = false }: AppTrialFlowProps) {
  const router = useRouter();
  const plannedUses = daysBetween(proofWindow.startDate, proofWindow.plannedEndDate);
  ...
  const [completedUses, setCompletedUses] = useState(Math.max(0, plannedUses - 1));
  ...
  async function finishTrial(mode: "upload" | "demo") {
    ...
    try {
      ...
      if (mode === "demo") {
        response = await fetch("/api/skin-analysis/tasks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ kind: "followup", scenario: completionScenario, allowCachedFallback: true }) });
      } else {
      ...
      const completion = await fetch(`/api/proof-windows/${proofWindow.id}/complete`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ scenario: completionScenario, followupAnalysisId: analysisPayload.data.analysis.id, completedUses, experience, majorConfounder: confounder, demoTimeJump: timeJumped }),
      });
  ...
  async function withdrawTrial() {
    setStatus("saving");
    setMessage("Ending this ProofWindow…");
    try {
      const response = await fetch(`/api/proof-windows/${proofWindow.id}`, { method: "DELETE" });
      const payload = await response.json() as { ok: boolean; error?: { message: string } };
      if (!response.ok || !payload.ok) throw new Error(payload.error?.message ?? "The ProofWindow could not be ended.");
      router.push("/app");
      router.refresh();
    } catch (error) {
      setStatus("error");
      setMessage(error instanceof Error ? error.message : "The ProofWindow could not be ended.");
    }
  }
  ...
  return <div className={`app-screen app-trial-screen ${demoMode ? "guided-consumer-screen" : ""}`}>
    ...
    {checkInSaved ? <section className="app-followup-card"><div><p className="app-kicker">{demoMode ? "Demo checkpoint" : followupReady ? "Day 14 checkpoint" : "Follow-up scheduled"}</p><h2>{timeJumped || followupReady ? "Complete your Day 14 follow-up" : "Your follow-up unlocks on Day 14"}</h2><p>{timeJumped ? "Use the same framing and lighting where possible. Because the duration is time-jumped, the resulting receipt remains synthetic even when the images are live." : followupReady ? "Use the same framing and lighting where possible so the comparison stays meaningful." : `Come back on ${proofWindow.plannedEndDate}. BeautyProof keeps the observation window honest and will unlock the follow-up then.`}</p></div>{demoMode && !timeJumped && !followupReady ? <button className="app-secondary-action" onClick={() => setTimeJumped(true)} type="button">Demo time jump → Day 14</button> : timeJumped || followupReady ? <div className="app-followup-controls"><label className="app-adherence-input"><span>Planned uses completed</span><input aria-label="Planned uses completed" max={plannedUses} min={0} onChange={(event) => setCompletedUses(Number(event.target.value))} type="number" value={completedUses} /><small>{Math.round(completedUses / plannedUses * 100)}% adherence · 80% required for a sponsored reward</small></label><label className="app-file-picker"><input accept="image/jpeg,image/png" aria-label="Choose or take a follow-up photo" onChange={(event) => setFollowupFile(event.target.files?.[0] ?? null)} type="file" /><span>{followupFile ? "Follow-up selected" : "Use your camera or photo library"}</span><strong>{followupFile ? followupFile.name : "Choose or take a follow-up photo"}</strong></label><button className="app-primary-action app-full-action" disabled={!followupFile || status === "analyzing"} onClick={() => finishTrial("upload")} type="button">{status === "analyzing" ? "Analyzing…" : "Analyze live follow-up"}<span>→</span></button>{demoMode ? <button className="app-secondary-action app-full-action" disabled={status === "analyzing"} onClick={() => finishTrial("demo")} type="button">Use simulated demo follow-up</button> : null}</div> : null}</section> : null}
    <section className="app-trial-exit"><button className="app-text-button" onClick={() => setShowWithdrawConfirm(true)} type="button">End this trial</button>{showWithdrawConfirm ? <div className="app-withdraw-confirm" role="alert"><div><strong>End this ProofWindow?</strong><p>Your saved baseline stays in your private space, but this trial will be marked withdrawn and cannot earn a campaign reward.</p></div><div><button className="app-secondary-action" disabled={status === "saving"} onClick={() => setShowWithdrawConfirm(false)} type="button">Keep trial</button><button className="app-danger-action" disabled={status === "saving"} onClick={withdrawTrial} type="button">{status === "saving" ? "Ending…" : "End trial"}</button></div></div> : null}</section>
