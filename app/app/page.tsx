...
export default async function AppHomePage() {
  ...
  try {
    [windows, receipts, coverage] = await Promise.all([repository.listWindows(), repository.listReceipts(), repository.coverage()]);
  } catch {
    dataAvailable = false;
  }
  ...
  return (
    <div className="app-screen app-dashboard">
      ...
      <div className="app-action-grid">
        ...
        <div className="app-action-card app-action-insight"><div className="app-action-icon">↗</div><span>Latest signal</span><h3>{baseline ? `${Math.round(baseline.metrics.hd_moisture)} moisture raw score` : "No baseline yet"}</h3><p>{baseline ? `${metricLabel("hd_moisture")} is stored with ${baseline.origin === "live_youcam" ? "live YouCam" : "cached-real YouCam"} provenance.` : "Your first scan will establish the reference point for a personal trial."}</p>{baseline ? <small>Captured {new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(new Date(baseline.capturedAt))}</small> : <Link href="/app/scan">Create baseline →</Link>}</div>
      </div>
